...
#   for it in range(ntimes):
    for it in range(-1,0):
        ...
        ssu = nc.ssu_east.data[it,:,:]
        ssv = nc.ssv_north.data[it,:,:]
...
#       ax.quiver(lon, lat, ssu, ssv, transform=vector_crs)
        ...
        ax.quiverkey(q, X=0.4, Y=-.1, U=0.5, label="Ocean speed, 0.5 m/s", labelpos='E')
...
#       outline_my_domain(ax,clon,clat)
        ...
        plt.suptitle('Ocean Surface currents (m/s)', fontsize=20)
