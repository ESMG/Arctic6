...
def plot_section(file_handle, record, variable, x=426, vmin=None, vmax=None, plot_grid=True, rep='linear'):
    ...
    """
    e = file_handle.variables['e'][record,:,72:82,426] # Vertical grid positions
    s = file_handle.variables[variable][record,:,72:82,x] # Scalar field to color
    x,z,q = m6toolbox.section2quadmesh(yq[71:82,426], e, s, representation=rep) # This yields three areas at twice the model resolution
...
#record = -1 # Last record
    ...
    plt.subplot(1,2,1); plot_section(z_file, record, x=427, variable='u', vmin=-2., vmax=2.); plt.title('U');
    plt.subplot(1,2,2); plot_section(z_file, record, variable='v', vmin=-2., vmax=2.); plt.title('V');
