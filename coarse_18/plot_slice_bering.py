...
z_file = netCDF4.Dataset('19800104.ocean_daily.nc')
...
def plot_section(file_handle, record, variable, y=358, vmin=None, vmax=None, plot_grid=True, rep='linear'):
    ...
    """
    e = file_handle.variables['e'][record,:,426,370:391] # Vertical grid positions
    s = file_handle.variables[variable][record,:,y,370:391] # Scalar field to color
    x,z,q = m6toolbox.section2quadmesh(xq[426,369:391], e, s, representation=rep) # This yields three areas at twice the model resolution
...
#   plt.subplot(1,2,2); plot_section(z_file, record, y=427, variable='v', vmin=-2., vmax=2.); plt.title('V');
    plt.subplot(2,2,1); plot_section(z_file, record, variable='u', vmin=-2., vmax=2.); plt.title('U');
    plt.subplot(2,2,2); plot_section(z_file, record, y=359, variable='v', vmin=-2., vmax=2.); plt.title('V');
    ...
    fname = myday.strftime('movie_Bering/frame_%(number)04d.png'%{'number': record})
    plt.savefig(fname)
    plt.close()
